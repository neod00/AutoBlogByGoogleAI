...
interface BlogResult {
  ...
  originalPost?: string; // ?´ë?ì§€ê°€ ?†ëŠ” ?ë³¸ ?¬ìŠ¤??}
...
const App: React.FC = () => {
    ...
    if (keywordParam && (autoParam === 'true' || autoParam === '1')) {
      ...
      (async () => {
        ...
        try {
          ...
          setError(`ê¸€ ?ì„±???¤íŒ¨?ˆìŠµ?ˆë‹¤: ${errorMessage}`);
  ...
  const handleImageConfirm = useCallback(async (imageKeywords: string[]) => {
    ...
    try {
      ...
      setBlogResult({
        ...
        originalPost: pendingBlogResult.post // ?ë³¸ ?¬ìŠ¤???€??      });
      setPendingBlogResult(null);
  ...
  const handleRegenerateImages = useCallback(() => {
    ...
    // ?´ë?ì§€ ?¬ìƒ?±ì„ ?„í•´ pendingBlogResultë¡??˜ëŒë¦?    // <figure> ?œê·¸?€ ê·??´ìš©??ëª¨ë‘ ?œê±°
  ...
  const LoadingSpinner: React.FC<{ phase: GenerationPhase }> = ({ phase }) => (
    <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400">
      ...
      {phase === 'generating' ? (
        <>
          <p className="mt-4 text-lg">ë¸”ë¡œê·?ê¸€???ì„±?˜ê³  ?ˆìŠµ?ˆë‹¤...</p>
          <p className="text-sm">Gemini AIê°€ ìµœì‹  ?´ìŠ¤ë¥?ê²€?‰í•˜ê³?ë¶„ì„?˜ëŠ” ???œê°„??ê±¸ë¦´ ???ˆìŠµ?ˆë‹¤.</p>
        </>
      ) : (
        <>
          <p className="mt-4 text-lg">?´ë?ì§€ë¥?ê²€?‰í•˜ê³??ˆìŠµ?ˆë‹¤...</p>
          <p className="text-sm">Pexels?ì„œ ê´€???´ë?ì§€ë¥?ì°¾ê³  ?ˆìŠµ?ˆë‹¤.</p>
        </>
      )}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100 flex flex-col p-4 md:p-8 transition-colors duration-300">
      <header className="w-full max-w-5xl mx-auto text-center mb-8 relative">
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">?´ìŠ¤ ê¸°ë°˜ ë¸”ë¡œê·??¬ìŠ¤???ì„±ê¸?/h1>
        <p className="text-lg text-gray-500 dark:text-gray-400 mt-2">Gemini AIë¥??¬ìš©?˜ì—¬ ìµœì‹  ?´ìŠ¤ë¡?ë¸”ë¡œê·?ê¸€ ?ë™ ?ì„±</p>
        <button
          onClick={toggleTheme}
      ...
      <main className="w-full max-w-5xl mx-auto flex flex-col flex-grow">
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <input
            ...
            placeholder="ë¸”ë¡œê·?ê¸€??ì£¼ì œê°€ ???¤ì›Œ?œë? ?…ë ¥?˜ì„¸??(?? AI ë°˜ë„ì²?"
          ...
          >
            ...
            <option value="day">ì§€??24?œê°„</option>
            <option value="week">ì§€??1ì£?/option>
            <option value="month">ì§€??1ê°œì›”</option>
            <option value="year">ì§€??1??/option>
          </select>
          <select
            value={template}
            ...
            className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 text-gray-900 dark:text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition"
            disabled={isLoading}
            aria-label="ë¸”ë¡œê·??œí”Œë¦?
          >
            <option value="default">ê¸°ë³¸ ?´ìŠ¤ ë¶„ì„</option>
            <option value="review">?œí’ˆ/?œë¹„??ë¦¬ë·°</option>
            <option value="interview">?„ë¬¸ê°€ ?¸í„°ë·??•ì‹</option>
            <option value="qa">Q&A ?•ì‹</option>
            <option value="investment">?¬ì?„ëµ ë³´ê³ ??/option>
          </select>
          <button
          ...
          {generationPhase === 'generating' || generationPhase === 'fetchingImages' ? (
            ...
            <div className="flex items-center justify-center h-full">
              <div className="text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/50 p-4 rounded-lg text-center">
                <p className="font-bold mb-2">?¤ë¥˜ ë°œìƒ</p>
                <p>{error}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center h-full text-gray-500 dark:text-gray-500">
              <p>?¤ì›Œ?œë? ?…ë ¥?˜ê³  'ë¸”ë¡œê·?ê¸€ ?ì„±' ë²„íŠ¼???´ë¦­?˜ì—¬ ?œì‘?˜ì„¸??</p>
            </div>
          )}
        </div>
      </main>
