...
// ── Category classification (DailyEnglishTips 영어 카테고리) ─────
const CATEGORIES = [
  "일상 회화",
  "직장 영어",
  "여행 영어",
...
// ── RSS related posts (dailyengtips.tistory.com) ────────────────
async function fetchRelatedPosts(category: string): Promise<{title: string, link: string}[]> {
  try {
    ...
    const res = await fetch("https://dailyengtips.tistory.com/rss");
    if (!res.ok) return [];
...
async function main() {
  ...
  const finalPrompt = `
    Role: You are a friendly Korean English-learning blog writer who explains practical everyday English expressions in a casual, relatable tone.
...
<div style="margin: 3rem 0; padding: 1.5rem; background-color: #eff6ff; border: 1px solid #bfdbfe; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
  <h3 style="margin-top: 0; color: #1e40af; font-size: 1.25rem; font-weight: 700; border-bottom: 2px solid #bfdbfe; padding-bottom: 0.5rem; margin-bottom: 1rem;">📖 영어 표현 더 알아보기</h3>
