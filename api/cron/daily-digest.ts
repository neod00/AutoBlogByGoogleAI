...
추천 기준:
1. 생활형과 실무형을 약 2:1로 섞습니다.
   - 생활형: 전기·난방 요금, 에너지 보조금·바우처·환급·포인트, 전기차·대중교통, 분리배출·자원순환처럼 일반 가정이 검색하는 기후·에너지 생활 정보
   - 실무형: CBAM, 제품 탄소발자국·Scope 3 산정, ESG 공시, 재생에너지 조달처럼 기업 실무 주제
2. 오늘 날짜 기준 앞으로 1~2개월 안에 검색이 몰릴 계절성 주제를 우선합니다. (예: 가을·겨울은 난방비·에너지바우처, 연말·연초는 전기차 보조금, 여름은 에어컨 전기세·누진제)
3. 최근 6~12개월 안에 실제 검색 수요나 정책/산업 변화가 확인되어야 합니다.
4. 너무 넓은 단어보다 2~5단어의 시드 키워드를 추천합니다.
5. 기존 시드와 완전히 같은 표현은 피하되, 성격이 맞는 핵심 주제는 더 구체적으로 갱신해도 됩니다.
6. 연예, 정치 공방, 공포 마케팅, 기후·에너지와 무관한 트래픽성 키워드는 제외합니다.
...
당신은 기후, ESG, 에너지 정책과 생활 정보 분야의 팩트체크 전문 AI입니다.

주제: "${seed}"

Google Search 결과를 사용해 최근 6~12개월 사이 실제로 확인되는 변화, 정책 발표, 규제 일정, 기업 대응, 지원사업, 시장 데이터를 조사하세요.
생활 정보 주제(요금, 보조금, 바우처, 환급, 포인트, 분리배출 등)라면 대상, 금액, 신청 기간, 신청처, 올해 시행 여부를 우선 확인하세요.

조사 기준:
1. 공식 기관, 정부, 국제기구, 기업 공시, 신뢰할 수 있는 언론/전문기관 자료를 우선합니다.
2. 과거 학습 기억에 의존하지 말고 검색 결과에서 확인되는 내용만 요약합니다.
3. 기후인사이트 독자가 실생활이나 실무에서 바로 활용할 만한 쟁점 3~5개를 bullet point로 정리합니다.
...
발굴 기준:
1. 기후·에너지 생활 정보(요금, 보조금, 바우처, 포인트, 분리배출 등) 또는 기후정책·ESG·기업 실무 주제여야 합니다. 생활 정보 주제는 template을 "qa"로 합니다.
